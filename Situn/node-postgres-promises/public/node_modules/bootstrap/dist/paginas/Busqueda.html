	   ...
	   <div class="col-md-6">
         <span class="text-danger"><h2>Tipo de Busqueda</h2></span>
          ...
          <input type="text" class="form45" id="buscar"  >
            <input type = "button" class="btn btn-default" id="buscar" value = "Buscar" onclick="busqueda()" >
...
if(B.value==false){
 ...
 function cargarTabla(arr)
 {
	...
	for(let i = 0; i < arr.length; i++){
		...
		tds += '<td>' + new Date(arr[i].tc_4).toLocaleDateString() + '</td>';
		tds += '<td>' + arr[i].tc_5 + '</td>';
		tds += '<td>' + arr[i].tc_6 + '</td>';
		tds += '<td>' + arr[i].tc_7 + '</td>';
		tds += '<td>' + arr[i].tc_8 + '</td>';
		tds += '<td>' + arr[i].tc_9 + '</td>';
		tds += '<td>' + arr[i].tc_10 + '</td>';
		tds += '</tr>';
	}	
	$("#tabla_busqueda").append(tds);
 }
