		...
		<tbody>
			<tr ng-repeat = "c in correspondencias">
					...
					<td> 
						...
						<button type="button" class="btn btn-danger" data-toggle="modal" data-target="#myModal" ng-click="obtenerEnlaces(c.tc_1)">Ver Enlaces</button>
					</td>
			</tr>
		</tbody>        
	</table>
...
function MB_01($scope)//ControllerAngular
 {
	...
	$scope.obtenerEnlaces	= n => asignarEnlaces($scope, n);
	$scope.enlaces = new Array();
  }
  
   app.controller('ControllerAngular', MB_01); 
 
 function asignarEnlaces($scope, cor)
 {
	arr = $scope.correspondencias;
	arr.forEach(e => 
				e.tc_1 == cor? (
				$scope.enlaces = e.enlaces,
				$("#idEnlace").text(e.tc_3) )
				:false);
 }
