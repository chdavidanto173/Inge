      ...
      <ul class="nav navbar-nav">
	   ...
	   <table class ="table">
			<tr>
				<td width = 45%>
					...
					<div class="col-md-8">
						...
						<div class="col-sm-2">
							<input type = "button"class="btn btn-default" id ="guardar" value = "Realizar Enlace" onclick = "insertTD()">
						</div>
						<div class="col-sm-3">
			...
			<!--tr>
						...
						<form class="form-horizontal">
							<div class="form-group">
								<label class="col-sm-2 control-label  text-danger">Codigo 1</label>
							...
							<div class="form-group">
								<label class="col-sm-2 control-label  text-danger">Codigo 2</label>
								<div class="col-sm-4">
...
function busqueda(id)
 {
	...
	.then(res => res.json())
		...
		.then(obj => cargarTabla(obj.data,id))  //Columna especifica obj.data[0].tp_1 // Nota: para pasar a JSON se usa JSON.stringify(obj.data[0].tp_1)
		.catch(err => console.log('Request failed', err));
	
	//console.log("Solicitada busqueda " + h3);
 }
 
 function cargarTabla(arr,fr)
 {
 $('#busqueda_enlace tbody').empty()
	let tds = '';
	for(let i = 0; i < arr.length; i++){
		tds += '<tr>'
		tds += '<td><button id = "asignar">' + '+' + '</button></td>';
		tds += '<td>' + arr[i].tc_3 + '</td>';
		tds += '<td>' + arr[i].tc_8 + '</td>';
		tds += '<td>' + arr[i].tc_5 + '</td>';
		tds += '<td>' + arr[i].tc_7 + '</td>';
		tds += '<td>' + arr[i].tc_1+ '</td>';
		tds += '</tr>';
	}
	
	$("#busqueda_enlace").append(tds);
	$( "#asignar" ).click( _ => $("#" + fr).val('jaj'));
 }	
 
 
 function insertTD(){
	let a4 = document.getElementById('Cod1').value;
	let b4 = document.getElementById('Cod2').value;
	console.log(a4 , b4);
